...
def _fetch_grants(cur):
    ...
    """)
    cols = [c[0] for c in cur.description]
    return [_grant_row(cols, r) for r in cur.fetchall()]


def _fetch_grant(cur, id):
    cur.execute("""
        SELECT id, title, description, scientific_codes, country, funding_type,
               academic_level, application_deadline, source_url, requirements_json,
               provider
    ...
    if not row:
        return None
    d = _grant_row([c[0] for c in cur.description], row)
    d['requirements_raw'] = (json.dumps(d['requirements_json'], ensure_ascii=False, indent=2)
                             if d['requirements_json'] else '')
    return d


def _render_admin_grants(grants, edit_g=None):
    return render_template('admin_grants.html', grants=grants, edit_g=edit_g,
                           funding_types=FUNDING_TYPES, academic_levels=ACADEMIC_LEVELS)


@grants_bp.route('/admin/grants', methods=['GET'])
@login_required
def admin_grants():
    from app import _require_admin
    _require_admin()
    from data import get_connection
    # Support ?edit=<id> as a shortcut into the edit form (used by the table link).
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                _ensure_schema(cur)
                grants = _fetch_grants(cur)
                if edit_id:
                    edit_g = _fetch_grant(cur, edit_id)
            conn.commit()
        finally:
            conn.close()
    except Exception:
        grants = []
    return _render_admin_grants(grants, edit_g)


@grants_bp.route('/admin/grants/add', methods=['GET'])
@login_required
def admin_grants_add_form():
    from app import _require_admin
    _require_admin()
    return _render_admin_grants([], edit_g=None)


@grants_bp.route('/admin/grants/add', methods=['POST'])
@login_required
def admin_grants_add():
    ...
    if not v['title']:
        ...
        return redirect('/admin/grants/add')
    try:
        conn = get_connection()
        try:
            with conn.cursor() as cur:
...
@grants_bp.route('/admin/grants/edit/<int:id>', methods=['GET'])
@login_required
def admin_grants_edit_form(id):
    from app import _require_admin
    _require_admin()
    from data import get_connection
    grants, edit_g = [], None
